...
class TestCleanNull:
    ...
    def test_clean_null(self, input_val, expected):
        ...
        from tools.woolworths.woolworths_setup import clean_null
...
class TestFetchStoreData:
    ...
    def _run_fetch(self, cleaned, json_path=None, temp_csv=None):
        ...
        with patch("tools.woolworths.woolworths_setup.JSON_DATA", json_path), \
             patch("tools.woolworths.woolworths_setup.CSV_STORES", temp_csv), \
             patch("tools.woolworths.woolworths_setup.requests.get", return_value=mock_resp):
            from tools.woolworths.woolworths_setup import fetch_store_data
    ...
    def test_store_list_writes_csv(self):
        ...
        try:
            ...
            mock_resp.json.return_value = fixture_data
            mock_resp.raise_for_status.return_value = None
            with patch("tools.woolworths.woolworths_setup.JSON_DATA", FIX_STORE_DATA_JSON), \
                 patch("tools.woolworths.woolworths_setup.CSV_STORES", temp_csv), \
                 patch("tools.woolworths.woolworths_setup.requests.get", return_value=mock_resp):
                from tools.woolworths.woolworths_setup import fetch_store_data
...
class TestCleaningLogic:
    ...
    def test_cleaned_drops_missing_coords(self):
        ...
        try:
            ...
            with patch("tools.woolworths.woolworths_setup.JSON_DATA", temp_json), \
                 patch("tools.woolworths.woolworths_setup.CSV_STORES", temp_csv), \
                 patch("tools.woolworths.woolworths_setup.requests.get", return_value=mock_resp):
                from tools.woolworths.woolworths_setup import fetch_store_data
...
class TestExcludedStores:
    ...
    def test_excluded_stores_constant_exists(self):
        ...
        from tools.woolworths.woolworths_setup import EXCLUDED_STORE_IDS
