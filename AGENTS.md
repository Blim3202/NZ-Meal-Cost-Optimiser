...
| `src/NZMealOptimiser/web/main.py` | FastAPI app + background-job optimisation API (`POST /optimise/jobs`, `GET /optimise/{id}` with per-company progress + event log) + legacy sync `/optimise` + frontend serving. Also: `/dish_filters` (curated keyword presets), `POST /optimise/{id}/reapply` (recalculate store costs from cached rows with edited ingredient filters — no new API calls), and `POST /optimise/{id}/filter_preview` (non-mutating dry-run powering the tuner's live match counts). HTTP logging middleware. Runs via `uvicorn NZMealOptimiser.web.main:app`. |
