...
- **Single-button flow**: one "Resolve and Compare" submit button (`white-space: nowrap` so the label stays on one line). `primaryAction` awaits `resolveSetup()` — which verifies dish + location (`GET /geocode` or GPS/picked lock) and requires ≥1 `/stores/nearby` store — and only then calls `runOptimise()` (`POST /optimise/jobs`). `resolveSetup` returns boolean; any intermediary failure aborts before a job starts, leaving the error banner, previous result, and map pins untouched. There is no stale-notice gate (every click re-resolves fresh); `staleNotice` survives only to hide the partial "Update ingredient prices" button until a fresh run. The green `is-ready` state additionally requires the address field to be blurred (`AddressAutocomplete` emits `focus`/`blur` → `addressFocused`), so the button never reads ready while an address is being typed. On run start the view smooth-scrolls to the `ProgressStrip` live-progress section (resolve failures return before the scroll, so the form + error stay in view).
- **Portions**: preset `Portions` input is `min="1" max="12"` (custom/shopping base portions were already `min="1"`); the backend accepts any int (`_scale_ingredients_to_portions` clamps with `max(1, …)`).
...
- Tooltip distinguishes pre-run pins ("Price preview — run Resolve and Compare") from completed runs ($ total used cost + ⚠ issue count).
...
| `GET /geocode?address=…` | resolve half of "Resolve and Compare" (non-GPS) | `{lat, lon, cached}` |
