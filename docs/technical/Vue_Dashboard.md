...
| `POST /optimise/{id}/ai_filter_preview` | **/test only** Filter tuner AI instruction | Universal dish-wide sentence → suggested `compiled_filters` + dry-run preview (ask-and-confirm before apply) |
| `POST /optimise/{id}/auto_cull_preview` | **/test only** Summary + Filter tuner Auto refine | Dish-wide auto-cull: up to 15 excludes + 15 brand_excludes per ingredient → additive preview (both pages) before apply |
