...
| 4 | `check_*.py` / `demo_*.py` (Phase 4) | Two-pass pipeline validated end-to-end for milk, beef mince, bread, cheese at multiple Auckland stores; per-store prices confirmed different (e.g. Shore City $9.49 vs Metro Auckland $26.99 beef mince) | Production-ready pipeline; the price-variation evidence (§7.2) validated the entire meal-cost-optimizer premise |
| 5 | `explore_category1_discovery.py` | 116 unique `category1` values discovered via 637 broad queries; 53 of them (`Dog`, `Cat`, `Pet`, `Baby`, `Household`, etc.) added to `NON_FOOD_CATEGORIES` and applied in Pass 1 across all Foodstuffs modules | Non-food category1 filtering eliminates pet food matching "beef mince" |
