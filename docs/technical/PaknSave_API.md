...
> **This document is the canonical reference for all Foodstuffs (Pak'nSave + New World) API integration in this project.** New World shares the same backend (`api-prod.prod.fsniwaikato.kiwi` + `api-prod.{paknsave,newworld}.co.nz/v1/edge`) and most of the same patterns, so [NewWorld_API.md](NewWorld_API.md) documents only New World-specific differences (banner value `"MNW"`, store count 148 vs 57, 2 mobile-only stores, and `NewWorldApp/4.32.0` User-Agent).
>
> **When extending either brand, update this file first, then mirror any brand-specific deltas into the other.** The cross-brand shared helpers in `src/NZMealOptimiser/pricing\optimiser_utils.py` (`foodstuffs_querier_edge`, `foodstuffs_querier_mobile`, `build_edge_row`, `build_mobile_row`, `parse_foodstuffs_volume_size`, `parse_foodstuffs_mobile_unit`, `_is_food_product`, `NON_FOOD_CATEGORIES`) are the single source of truth for the Algolia two-pass pipeline and CSV row construction.
>
> ### What's shared
>
...
> - **Algolia indices** — same 3 working indices (`products-index`, `products-index-popularity-asc`, `products-index-popularity-desc`); the other 8 enumerated patterns return HTTP 500.
...
>
> **Pattern:** When editing either file, ask "is this the Foodstuffs contract, or a brand-specific detail?" If the former, edit **only** this document. If the latter, edit **both** documents but mirror the change here first.
...
- **Pak'nSave API client**: `src/NZMealOptimiser/pricing/paknsave_api.py` — `PaknSaveAPI(backend="edge"|"mobile")`.
- **Cross-brand shared logic**: `src/NZMealOptimiser/pricing/optimiser_utils.py` — `foodstuffs_querier_edge`, `foodstuffs_querier_mobile`, `build_edge_row`, `build_mobile_row`, parsing, geocoding, haversine, dish lookup, Phase 2 CSV readers, and append-only CSV writers.
...
Pipeline)**. Reference implementation: `src/NZMealOptimiser/pricing/paknsave_api.py`
...
`optimise`, etc.) live in `src/NZMealOptimiser/pricing/optimiser_utils.py`. The CLI optimisers
`tools/paknsave/paknsave_optimiser_edge.py` and `tools/paknsave/paknsave_optimiser_mobile.py`
...
**Default source: Edge API.** `tools/paknsave/paknsave_setup.py` defaults to
`source="edge"` (57 stores for Pak'nSave). The store builder, like the query layer, is
Edge-first.

```python
from tools.paknsave.paknsave_setup import fetch_stores, clean_stores, run_full_setup
...
CLI: `python -m tools.paknsave.paknsave_setup [--source edge|mobile|store_finder] [--cleaned true|false]`
(defaults: `--source edge --cleaned true`).

**Legacy scripts:** the old standalone `fetch_stores` script was **deprecated** and folded
into `tools/paknsave/paknsave_setup.py` — use `fetch_stores()` from there instead.
...
```python
from NZMealOptimiser.pricing.paknsave_api import PaknSaveAPI, PaknSaveEdgeAPI, PaknSaveMobileAPI
...
**Edge** (`tools/paknsave/paknsave_optimiser_edge.py`):
...
**Mobile** (`tools/paknsave/paknsave_optimiser_mobile.py`):
...
```python
from NZMealOptimiser.pricing.paknsave_api import PaknSaveEdgeAPI
from tools.paknsave.paknsave_setup import load_stores
...
```python
from NZMealOptimiser.pricing.paknsave_api import PaknSaveMobileAPI
...
`src/NZMealOptimiser/pricing/optimiser_utils.py`. LLM-backed
dish generation available via `src/NZMealOptimiser/llm/llm_utils.py`.
...
from `data/dishes.json` via `src/NZMealOptimiser/pricing/optimiser_utils.py` (via `get_ingredients()`).
...
```powershell
python -m tools.paknsave.paknsave_optimiser_edge "588 Chapel Road, East Tāmaki, Auckland 2016" "spaghetti bolognese"
```

**Mobile API Optimiser (Fallback — single-pass):**
```powershell
python -m tools.paknsave.paknsave_optimiser_mobile "588 Chapel Road, East Tāmaki, Auckland 2016" "spaghetti bolognese"
```
