...
The repo is an installable **src-layout Python package** (`pyproject.toml`,
editable install via `pip install -e .`). Library code lives under
`src/NZMealOptimiser/`, the CLI layer under `tools/`, tests under `tests/`,
exploration scripts under `exploration/`, and docs under `docs/`.
...
tools/                 # CLI layer: optimisers, setup, llm_validate, llm_interactive, initialize_full_results
tests/                 # all test suites + fixtures
exploration/           # per-brand exploration scripts
docs/project/          # decision.md, design.md, logs.md
docs/technical/        # PaknSave_API.md, NewWorld_API.md, Woolworths_API.md, LLM_Pipeline.md, FastAPI.md
```

No manual path bootstrap is needed — imports resolve as
`from NZMealOptimiser.pricing.optimiser_utils import ...`. Run CLIs as
`python -m tools.<brand>.<module>`. `data/` stays at the repo root;
`DATA_DIR = PROJECT_ROOT / "data"` is resolved in `src/NZMealOptimiser/__init__.py`.
...
```bash
python -m tools.paknsave.paknsave_optimiser_mobile "Botany Town Centre, Auckland" "spaghetti bolognese"
```
...
3. **Build process** (`tools/paknsave/paknsave_setup.py`): unified callable module with three sources:
...
CLI:
```
python -m tools.paknsave.paknsave_setup [store_finder|edge|mobile]
```

Module:
```python
from tools.paknsave.paknsave_setup import fetch_stores, run_full_setup
...
from NZMealOptimiser.pricing.optimiser_utils import get_ingredients
...
- `DISHES` dict loaded from `data/dishes.json` via `src/NZMealOptimiser/pricing/optimiser_utils.py` (dict format with quantity/unit/search_term)
...
- LLM-backed dish generation available via `src/NZMealOptimiser/llm/llm_utils.py`
...
python -m tools.paknsave.paknsave_setup store_finder  # Store-finder page (60 stores)
python -m tools.paknsave.paknsave_setup mobile         # Mobile API (60 stores, legacy)
```
...
from NZMealOptimiser.pricing.optimiser_utils import woolworths_querier, optimise
