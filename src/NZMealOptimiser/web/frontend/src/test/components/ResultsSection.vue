<template>
  <section v-if="result" class="results-area">
    ...
    <section class="panel">
      ...
      <div v-else class="detail-scroll"><table class="results-table"><thead><tr><th>Company</th><th>Store</th><th>Search Term</th><th>Returned Product</th><th>Brand</th><th>Recipe Needed</th><th>Price</th><th>Pack Size</th><th>Cost Per Unit</th><th>Used Price</th><th>Purch Qty</th><th>Purch Cost</th><th>Status</th><th>SKU</th></tr></thead><tbody><tr v-for="row in filteredRows" :key="`${row.company}-${row.store}-${row.sku}-${row.search_ingredient}`" :class="{ 'row-invalid': row.valid_ingredient === false }"><td><span class="badge" :class="badgeClass(row.company)">{{ companyLabel(row.company) }}</span></td><td>{{ row.store || '-' }}</td><td>{{ row.search_ingredient || '-' }}</td><td>{{ row.returned_ingredient || '-' }}</td><td>{{ row.brand || '-' }}</td><td>{{ recipe(row) }}</td><td>{{ money(row.price) }}</td><td>{{ pack(row) }}</td><td>{{ unitPrice(row) }}</td><td>{{ usedPrice(row) }}</td><td>{{ row.purchase_quantity || 0 }}</td><td>{{ money(row.purchase_price) }}</td><td><span class="status" :class="statusClass(row.status)">{{ statusLabel(row.status) || '-' }}</span><span v-if="row.valid_ingredient === false" class="status status-filtered" title="Excluded from store costs by ingredient filters">{{ row.filter_reason ? `filtered · ${row.filter_reason}` : 'filtered' }}</span></td><td>{{ row.sku || '-' }}</td></tr></tbody></table></div>
    </section>
  </section>
</template>
