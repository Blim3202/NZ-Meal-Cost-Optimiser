<template>
  <div class="map-wrap">
    ...
    <p v-if="!stores.length" class="map-hint">{{ origin ? 'No stores in range. Widen the distance or move the origin.' : 'Set an address, GPS, or map pin to preview nearby stores.' }}</p>
  ...
  setup(props, { emit }) {
    ...
    function storeTooltip(store) {
      ...
      const costLine = cost === null || cost === undefined || cost === ''
        ? 'Price preview: run Resolve and Compare'
