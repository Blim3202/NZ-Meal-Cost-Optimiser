<template>
  <section v-if="job.status !== 'idle'" class="progress-strip">
    <div class="strip-top">
      <div class="strip-phase"><p class="eyebrow">Live progress</p><strong>{{ job.phase || 'Working…' }}</strong></div>
      <div class="overall-bar" :class="{ indeterminate: running && !job.total_tasks }"><div class="overall-fill" :style="{ width: pct + '%' }"></div></div>
      <div class="job-meta">
