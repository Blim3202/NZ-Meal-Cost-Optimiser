  ...
  <div class="dish-builder">
    <template v-if="mode === 'locked'">
      ...
      <ul v-else class="ingredient-list">
        <li v-for="(ing, index) in ingredients" :key="index"><span class="ing-name">{{ ing.search_term }}</span><span class="ing-qty">{{ displayQty(ing) }}</span></li>
      </ul>
    </template>
    <template v-else>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    return { APPROX_UNITS, UNIT_GROUPS, scaleFactor, trimFactor, patch, needsApprox, hasApprox, isDuplicate, isBlank, isKnownUnit, unitValue, displayQty };
  },
};
</script>
