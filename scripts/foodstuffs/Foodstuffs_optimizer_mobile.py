...
Usage:
    python -m scripts.foodstuffs.Foodstuffs_optimizer_mobile "Botany Town Centre, Auckland" "spaghetti bolognese"
    python -m scripts.foodstuffs.Foodstuffs_optimizer_mobile "Botany Town Centre, Auckland" "spaghetti bolognese" newworld
"""
import sys
import time
import re
...
OUTPUT_CSV = {
    "paknsave": DATA_DIR / "paknsave_mobile_latest_results.csv",
    "newworld": DATA_DIR / "newworld_mobile_latest_results.csv",
}
MAX_DISPLAY = 80


def search_ingredient_at_store(
    api: FoodstuffsMobileAPI,
    store_id: str,
    ingredient: str,
) -> list[dict]:
    try:
    ...
    enriched = []
    for p in products:
        price = FoodstuffsMobileAPI.extract_price(p)
        if price is None:
            continue
        enriched.append({
            "product_id": p.get("productId"),
            "name": FoodstuffsMobileAPI.get_product_name(p),
            "size": FoodstuffsMobileAPI.get_product_size(p),
            "price": price,
            "unit_price": FoodstuffsMobileAPI.extract_unit_price(p),
            "is_promo": False,
        })
    return enriched
...
def optimize_dish_mobile(
    ...
    title = brand.title()
    print("=" * 70)
    print(f"{title} Mobile API Optimizer -- {dish_name.title()}")
    print("=" * 70)
    ...
    api = FoodstuffsMobileAPI(brand=brand)
    api._ensure_token()
        ...
        for ing in ingredients:
            ...
            if products:
                ...
                if best:
                    ...
                    print(f"${best['price']:.2f}{unit_str}  --  {name_display}")
                    all_results.append({
                        "store": store_name,
                        "store_id": store_id,
                        "brand": brand,
...
def main():
    ...
    try:
        optimize_dish_mobile(address, dish, brand=brand)
    except Exception as e:
        print(f"\n[ERROR] {e}")
        sys.exit(1)
