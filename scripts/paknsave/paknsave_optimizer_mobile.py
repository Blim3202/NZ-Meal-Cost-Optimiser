...
Usage:
    python -m scripts.paknsave.paknsave_optimizer_mobile "Botany Town Centre, Auckland" "spaghetti bolognese"
"""
...
OUTPUT_CSV = DATA_DIR / "paknsave_mobile_latest_results.csv"
...
def search_ingredient_at_store(
    ...
    enriched = []
    for p in products:
        price = PaknSaveMobileAPI.extract_price(p)
        if price is None:
            continue
        enriched.append({
            "product_id": p.get("productId"),
            "name": PaknSaveMobileAPI.get_product_name(p),
            "size": PaknSaveMobileAPI.get_product_size(p),
            "price": price,
            "unit_price": PaknSaveMobileAPI.extract_unit_price(p),
...
def optimize_dish_mobile(
    ...
    """
    print("=" * 70)
    print(f"Pak'nSave Mobile API Optimizer -- {dish_name.title()}")
    print("=" * 70)
