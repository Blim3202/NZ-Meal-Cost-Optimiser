...
each (source, cleaned) permutation in tools/newworld/newworld_setup.

Sources:   edge, mobile
Cleaned:   true, false
Total:     2 sources x 2 cleaned = 4 permutations
...
This script was previously tests/combined/test_newworld_setup_permutations.py —
moved to scripts/api_claims/ because it hits the live API on every invocation
and was masquerading as a pytest test.

NOTE: run_full_setup mutates data/newworld_stores.csv as a side effect.
Back up the file if you care about its current contents.

Source docs: docs/technical/NewWorld_API.md §9.
...
def check_permutation(source, cleaned):
    ...
    try:
        ...
        import pandas as pd
        csv_df = pd.read_csv(str(DATA_DIR / "newworld_stores.csv"))
...
def main():
    print("=" * 60)
    print("New World Setup Permutation Probe")
