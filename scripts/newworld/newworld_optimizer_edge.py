...
Usage:
    python -m scripts.newworld.newworld_optimizer_edge "Botany Town Centre, Auckland" "spaghetti bolognese"
"""
...
OUTPUT_CSV = DATA_DIR / "newworld_latest_results.csv"
...
def search_ingredient_at_store(
    ...
    enriched = []
    for p in products:
        price = NewWorldEdgeAPI.extract_price(p)
        if price is None:
            continue
        enriched.append({
            "product_id": p.get("productID"),
            "name": NewWorldEdgeAPI.get_product_name(p),
            "size": NewWorldEdgeAPI.get_product_size(p),
            "price": price,
            "unit_price": NewWorldEdgeAPI.extract_unit_price(p),
...
def optimize_dish_edge(
    ...
    """
    print("=" * 70)
    print(f"New World Edge API Optimizer -- {dish_name.title()}")
    print("=" * 70)
